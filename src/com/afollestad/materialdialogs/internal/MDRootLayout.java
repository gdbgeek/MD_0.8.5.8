...
public class MDRootLayout extends ViewGroup {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, RX.styleable.MDRootLayout, defStyleAttr, 0);
        mReducePaddingNoTitleNoButtons = a.getBoolean(RX.styleable.MDRootLayout_md_reduce_padding_no_title_no_buttons, true);
        a.recycle();

        mNoTitlePaddingFull = r.getDimensionPixelSize(RX.dimen.md_notitle_vertical_padding);
        mButtonPaddingFull = r.getDimensionPixelSize(RX.dimen.md_button_frame_vertical_padding);

        mButtonHorizontalEdgeMargin = r.getDimensionPixelSize(RX.dimen.md_button_padding_frame_side);
        mButtonBarHeight = r.getDimensionPixelSize(RX.dimen.md_button_height);

        mDividerPaint = new Paint();
        mDividerWidth = r.getDimensionPixelSize(RX.dimen.md_divider_height);
        mDividerPaint.setColor(DialogUtils.resolveColor(context, RX.attr.md_divider_color));
    ...
    public void onFinishInflate() {
        ...
        for (int i = 0; i < getChildCount(); i++) {
            View v = getChildAt(i);
            if (v.getId() == RX.id.titleFrame) {
                mTitleBar = v;
            } else if (v.getId() == RX.id.buttonDefaultNeutral) {
                mButtons[INDEX_NEUTRAL] = (MDButton) v;
            } else if (v.getId() == RX.id.buttonDefaultNegative) {
                mButtons[INDEX_NEGATIVE] = (MDButton) v;
            } else if (v.getId() == RX.id.buttonDefaultPositive) {
    ...
    public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (!mForceStack) {
            ...
            int buttonBarPadding = getContext().getResources()
                    .getDimensionPixelSize(RX.dimen.md_neutral_button_margin);
