...
class PrefUtil {
    ...
    public static void setLayoutResource(@NonNull Context context, @NonNull Preference preference, @Nullable AttributeSet attrs) {
        ...
        if (attrs != null) {
            TypedArray a = context.getTheme().obtainStyledAttributes(attrs, RX.styleable.Preference, 0, 0);
            try {
                useStockLayout = a.getBoolean(RX.styleable.Preference_useStockLayout, false);
            } finally {
                a.recycle();
            }
        }
        ...
        if (!foundLayout && !useStockLayout)
            preference.setLayoutResource(RX.layout.md_preference_custom);
    }
