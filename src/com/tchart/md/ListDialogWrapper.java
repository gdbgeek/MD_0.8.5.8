...
@BA.ShortName("MD_ListDialog")
@ActivityObject

@Events(values={"ButtonPress(Result As Int)", "Show", "Cancel", "Dismiss","ItemSelected(Index As Int, Text As String)"})

public class ListDialogWrapper
{	
	private BA ba;
	...
	public void Show(String title, List items, String positiveText, String negativeText)
	{
		if (ba.subExists(EventName + "_itemselected"))
		{
			MD.itemsCallback(new MaterialDialog.ListCallback()
			{
				...
				{
					ba.raiseEventFromUI(this, EventName + "_itemselected",which,text.toString());
                }
            }
			);
		}
